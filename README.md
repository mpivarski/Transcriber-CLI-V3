...
Sparse Matrix feature which creates a sparse matrix including missing data generated by the LLM and columns listing what the missing entries are being added for each specimen. This is designed for museum specimen digitization workflows where an LLM fills in missing fields (collector names, dates, geographic info, etc.) and a human curator must review and import only the changed data.  See the [Sparse Matrix README](https://github.com/mpivarski/Transcriber-CLI-V3/blob/main/SpareMatrix/README.md) for more information.
